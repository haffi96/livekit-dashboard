...
export function DataTracksSection() {
  ...
  const totalTrackCount =
    remoteTracks.filter((track) => track.isPublished).length +
    localTracks.length;
  const hasVisibleRemoteTracks = remoteTracks.some(
  ...
  return (
    <section className="rounded-xl border border-cyan-950/60 bg-linear-to-br from-cyan-950/20 via-neutral-950 to-neutral-950 p-4">
      <Button
        ...
        className="h-auto w-full items-start justify-between gap-3 px-0 py-1 text-left whitespace-normal text-neutral-100 hover:bg-transparent hover:text-neutral-50"
      >
        <div className="min-w-0 flex-1 pr-2">
          <div className="text-sm font-semibold">Data Tracks</div>
          <p className="mt-1 text-xs leading-5 text-neutral-500">
            Published lossy tracks with explicit subscribe, publish, and
            unpublish lifecycle.
          </p>
        </div>
